    ...
    while ((rowMatch = rowPattern.exec(html)) !== null) {
      ...
      if (cells.length > 0 && cells[0]) {
        results.push({
          ...
          name: cells[0],
          provider: 'ktuvit',
          downloadUrl: `https://www.ktuvit.me/Services/DownloadFile.ashx?DownloadIdentifier=${subId}`,
        });
      }
    }
...
async function searchOpenSubs(imdbId, season, episode, apiKey) {
  ...
  try {
    ...
    const results = (data.data || []).map(item => {
      ...
      const fileId = (attr.files && attr.files[0]) ? attr.files[0].file_id : item.id;
      return {
        id: String(fileId),
        name: attr.release || (attr.feature_details || {}).title || '',
        provider: 'opensubtitles',
        downloadUrl: '', // Requires separate download API call
      };
    }).filter(s => s.name);
...
module.exports = { searchWizdom, searchKtuvit, searchOpenSubs, searchAll };
