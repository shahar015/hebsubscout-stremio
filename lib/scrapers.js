...
async function scrapeStremio(baseUrl, providerName, type, id) {
  try {
    ...
    return (data.streams || []).map(stream => {
      ...
      if (!releaseName) {
        // Clean unicode and extract first line
        const clean = title.replace(/[\u2600-\u27FF]/g, '').replace(/\s+/g, ' ').trim();
        releaseName = clean.split('\n')[0] || clean;
      }

      let size = '';
      if (hints.videoSize) {
        size = formatSize(hints.videoSize);
      } else {
        // Try extracting from title lines
        for (const line of title.split('\n')) {
          if (/gb|mb/i.test(line)) { size = line.replace(/\s+/g, ' ').trim(); break; }
        }
      }
      ...
      return {
        ...
        // Keep original stream for passing through to Stremio
        _original: stream,
      };
    });
  } catch (e) {
