...
async function scrapeStremio(baseUrl, providerName, type, id) {
  try {
    ...
    console.error(`[HebSubScout] ${providerName} scrape error:`, e.name, e.message);
    return [];
  }
}

async function scrapeTorrentio(type, id, configUrl) {
  // If user provides their Torrentio config URL (with RD token), use that
  const baseUrl = configUrl || 'https://torrentio.strem.fun';
  return scrapeStremio(baseUrl, 'Torrentio', type, id);
}
...
async function scrapeAll(type, id, config = {}) {
  const promises = [scrapeTorrentio(type, id, config.torrentioUrl)];
